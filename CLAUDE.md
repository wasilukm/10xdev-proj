...
An **independent** uv project (own `pyproject.toml`, `uv.lock`, `.venv`) wrapping the [Claude Agent SDK](https://code.claude.com/docs/en/agent-sdk/overview) as a diff-only, agentic PR reviewer wired into GitHub Actions. It is **not** a uv workspace member and is not a dependency of the root `envbooker` project — this keeps `claude-agent-sdk` out of the Django app's Railway deploy image. Root `pyproject.toml`'s `[tool.mypy]` excludes `^packages/` from the **root** mypy pass only (its own dependencies aren't installed in the root `.venv` that the repo-wide mypy gate runs against). The subpackage has its own `[tool.mypy]` (`strict = true`, no django plugin) and is type-checked separately: `cd packages/code_reviewer && uv run mypy .`, wired into `lefthook.yml`'s `pre-commit` as `typecheck-reviewer` alongside the root `typecheck` command. See `packages/code_reviewer/README.md` for install/run/auth — it needs an `ANTHROPIC_API_KEY` repo secret in CI. Ruff still applies repo-wide: the subpackage has no `[tool.ruff]` table of its own, so ruff's config discovery walks up to root's.

`ai-cr:passed` must never become a required status check in branch protection — the review is advisory, and the label may only subtract (`ai-cr:failed` adds friction; `ai-cr:passed` grants nothing). See `packages/code_reviewer/README.md`'s "Advisory posture" section.
