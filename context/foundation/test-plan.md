...
- **Static files / JS must load (htmx)**: production storage is whitenoise `CompressedManifestStaticFilesStorage`, whose `{% static %}` URLs are content-hashed and only exist after `collectstatic`. `live_server` serves static via the staticfiles *finders* (source names), so the hashed names 404, htmx never loads, and the `<form hx-post>` silently does a native full-page GET — every HTMX assertion then fails for a non-obvious reason. The autouse `_servable_static` fixture in `conftest.py` swaps `STORAGES["staticfiles"]` to the plain `StaticFilesStorage` for the test run so `{% static %}` emits finder-resolvable names. Any test that depends on JS needs this (it is autouse, so you get it for free); do not remove it. Note: the smoke test asserts nothing about JS and will pass even when static serving is broken — it is not a guard for this.
