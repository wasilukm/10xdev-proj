...
class ComputeEndTest(TestCase):
    ...
    def test_preset_1h(self):
        self.assertEqual(
            compute_end(self.env, self.start, "1h"), self.start + timedelta(hours=1)
        )

    def test_preset_2h(self):
        self.assertEqual(
            compute_end(self.env, self.start, "2h"), self.start + timedelta(hours=2)
        )

    def test_preset_4h(self):
        self.assertEqual(
            compute_end(self.env, self.start, "4h"), self.start + timedelta(hours=4)
        )
    ...
    def test_until_next_no_reservation_caps_at_max(self):
        ...
        self.assertEqual(
            compute_end(self.env, self.start, "until_next"), self.start + MAX_DURATION
        )
